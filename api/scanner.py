...
def scan_image(image_bytes: bytes) -> bytes:
    ...
    img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)     # decode array ke citra OpenCV warna (BGR)
    ...
    if h_asli > tinggi_maks:                        # jika gambar lebih tinggi dari batas maks
        ...
        img = cv2.resize(img, (int(img.shape[1] / rasio), tinggi_maks))  # resize lebar seperlimal tinggi

    # ubah jadi grayscale
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)    # konversi citra warna ke grayscale

    # blur
    blur = cv2.GaussianBlur(gray, (5,5), 0)         # smooth gambar dengan Gaussian Blur kernel 5x5
    ...
    edged = cv2.dilate(edged, kernel, iterations=1) # melapisi tepi untuk menyambungkan kontur putus

    kontur, _ = cv2.findContours(edged.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)  # cari semua kontur di citra edged
    kontur = sorted(kontur, key=cv2.contourArea, reverse=True)[:5]  # urutkan kontur berdasarkan luas, ambil 5 terbesar
    ...
    for c in kontur:                                # iterasi tiap kontur terbesar
        ...
        approx = cv2.approxPolyDP(c, 0.02 * keliling, True)  # pendekatan poligon dengan toleransi 2% dari keliling
    ...
    if titik_sudut is None and len(kontur) > 0:    # jika tidak ditemukan 4 titus tapi ada kontur
        ...
        menyentuh_bawah = y_maks >= (h - 1 - toleransi_tepi)  # apakah kontur menyentuh tepi bawah (dengan toleransi)

        if menyentuh_bawah:                         # jika menyentuh tepi bawah
            y_tengah = h / 2                        # garis tengah gambar secara vertikal
            titik_atas = kontur_terbesar[kontur_terbesar[:, 1] < y_tengah]  # titik di atas tengah
            titik_bawah = kontur_terbesar[kontur_terbesar[:, 1] >= y_tengah] # titik di bawah atau pada tengah

            if len(titik_atas) > 0 and len(titik_bawah) > 0:  # pastikan ada titik di keduanya
                s_atas = titik_atas[:, 0] + titik_atas[:, 1]  # jumlah x+y untuk setiap titik Atas
                diff_atas = titik_atas[:, 0] - titik_atas[:, 1] # perbedaan x-y untuk titik Atas

                top_left = titik_atas[np.argmin(s_atas)]       # titik kiri atas (x+y terkecil)
                top_right = titik_atas[np.argmax(diff_atas)]   # titik kanan atas (x-y terkecil)

                bottom_left_x = titik_bawah[np.argmin(titik_bawah[:, 0])][0]  # lebar kiri bawah
                bottom_right_x = titik_bawah[np.argmax(titik_bawah[:, 0])][0]  # lebar kanan bawah

                bottom_left = np.array([bottom_left_x, h - 1])   # titik kiri bawah (dipotong ke bawah)
                bottom_right = np.array([bottom_right_x, h - 1]) # kanan bawah (dipotong ke bawah)

                titik_sudut = np.array([top_left, top_right, bottom_right, bottom_left]) # susun ulang 4 sudut
                print("Sisi bawah dokumen terpotong frame, tepi bawah gambar dipakai sebagai pengganti.") # catatan log

    # --- Fallback terakhir: pakai seluruh frame ---
    if titik_sudut is None:                         # jika tidak ditemukan pun sudut punapa
        print("Kontur 4 sisi tidak ditemukan, fallback pakai seluruh frame.") # catatan log
        titik_sudut = np.array([[0, 0], [w, 0], [w, h], [0, h]])  # gunakan seluruh frame sebagai ROI

    
    def urutkan_titik(pts):
        """Urutkan 4 titik jadi: top-left, top-right, bottom-right, bottom-left."""
        rect = np.zeros((4, 2), dtype="float32")  # array kosong untuk menampung 4 titur urut
    ...
    def perspective_transform(image, pts):
        ...
        (tl, tr, br, bl) = rect                     #pecah ke 4 titik: kiri atas, kanan atas, kanan bawah, kiri bawah

        lebar_bawah = np.linalg.norm(br - bl)       # hitung panjang sisi kiri bawah
        lebar_atas = np.linalg.norm(tr - tl)        # hitung panjang sisi atas
        lebar_maks = int(max(lebar_bawah, lebar_atas))  # lebar maksimum dari atas dan bawah
        ...
        dst = np.array([                            # titik tujuan untuk pemetaan perspektif
            [0, 0],                                 # kiri atas
        ...
        M = cv2.getPerspectiveTransform(rect, dst)  # hitung matriks transformasi perspektif
        hasil = cv2.warpPerspective(image, M, (lebar_maks, tinggi_maks))  # terapkan transformasi
    ...
    titik_sudut_asli = titik_sudut.astype("float32") * rasio  # kalikan koordinat sudut dengan rasio resize

    hasil_warna = perspective_transform(img, titik_sudut_asli)  # terapkan transformasi perspektif ke citra asli

    hasil_gray = cv2.cvtColor(hasil_warna, cv2.COLOR_BGR2GRAY)  # konversi hasil warna ke grayscale lagi (sekaligus memastikan format)
    ...
    scanned = cv2.adaptiveThreshold(
        ...
        15                                          # konstanta C dikurangi dari mean
    )
    ...
    # 4. Cleanup akhir
    final = cv2.medianBlur(scanned, 3)            # blur median 3x3 untuk menghilangkan noise pepper-and-salt
